...
To calculate delta for a neuron, we use vector multiplication and take the product of the vector $D_{k}$ of deltas of the previous layer and $W^t$, which is the transpose of the adjacency matrix of the weights between this neuron and each neuron in the other layer. Then, we multiply this matrix by Oj$\otimes$(I-Oj), which is the product of the Output vector, Oj, and (I-Oj). This shows the formula for Dj is correct.
...
\section{Dependence on Parameters} \label{sec:param}
    ...
    We found that training rates greater than 0.5 or less than 0.1 had poor results, while training rates in the range of 0.1 to 0.5 gave the best results with little variation within the range. Similary for weights the optimal maximum absolute value of the initial weights was between 0.5 and 0.7
...
Although the ranges discussed in the parameter study were most consistent, the network that produced the best results had the following values: a single hidden layer with 200 neurons, a max initial weights of 0.1756 and a training rate of 1.5. This network got the numbers right 92.1\% of the time, with a computed average error of 10.27\%. Meanwhile, the network that performed the worst used three hidden layers with 100, 25, and 5 neurons respectively, max initial weights of 1.5 and training rate of 0.1. This makes sense as we generally saw poor performance with increasing numbers of hidden layers. Combined with the fact that the initial weights are highly random and the training rate is too small to readjust them, led to a very poorly trained network yielding the correct values only 10.02\% of the time. In general, if we were to achieve higher confidence, we could use higher resolution images. Although it would bring the computation time up, having such low resolution images decreases the number of meaningful characterizations the computer can make of each digit. Another issue was that we never played around with different activation functions. Perhaps a different activation function would yield better results. Lastly, we had talked about biasing the input. We know that for the most part there is no relevant data near the corners of the image, thus by biasing against the corners of the image and biasing for the center of the image, we'd train the computer to form connections on the relevant parts of the image, perhaps leading to lower errors.
...
For the most part, we all worked hand-in-hand while we worked on this project. Before we wrote any code, all of us met and discussed everything we needed to do in order to move forward with the project; the actual writing of the code is trivial. We met 3 times for about 7 hours total. We also had a text group chat which we communicated over extensively. We created a github repository (https://github.com/TTiole/MAT128Project2) where all the resources (code, data) are available to run the entirety of our project on any computer. The repository also contains this report and figures. As for who coded what, Francis coded the Neuron code in its entirety (she also wrote a lot of the report). Eli wrote the foundation for the Network, NetworkIteration code, while Nikos finished Network and NetworkIteration; and wrote ForwardPass, NetworkBackpropagation, TestNetwork and handled the parameter study. But again, everyone contributed to the designing of the neural network and the implementation through our meetings.

\subsection{Code}

\subsubsection{Design}

When we were talking about how this was going to be implemented, it was very clear that we would want functions for each step in order to structure our code in a way that is readable. The "main" file is Project2.m. This is the file that you'd run in order to see the project in its entirety. It goes through reading the data, performing the parameter study and plotting the results. An instance of a neural network is represented by Network.m, which takes in the parameters discussed (training rate, max absolute value of initial weights, number of neurons per layer). In this function, NetworkIteration is called to go through a single image in order to train the Network and TestNetwork is called in order to test the network in its entirety. Within NetworkIteration, ForwardPass and NetworkBackpropagate perform the two training steps. The former of which uses Neuron, which calculates the Neuron values given the weights and neuron values of the previous layers.

\subsubsection{Project2.m}
\lstinputlisting[language=Matlab]{../Code/Project2.m}
\subsubsection{Network.m}
\lstinputlisting[language=Matlab]{../Code/Network.m}
\subsubsection{NetworkIteration.m}
\lstinputlisting[language=Matlab]{../Code/NetworkIteration.m}
\subsubsection{ForwardPass.m}
\lstinputlisting[language=Matlab]{../Code/ForwardPass.m}
\subsubsection{NetworkBackpropagate.m}
\lstinputlisting[language=Matlab]{../Code/NetworkBackpropagate.m}
\subsubsection{Neuron.m}
\lstinputlisting[language=Matlab]{../Code/Neuron.m}
\subsubsection{TestNetwork.m}
\lstinputlisting[language=Matlab]{../Code/TestNetwork.m}
\subsubsection{readMNIST.m}
\lstinputlisting[language=Matlab]{../Code/readMNIST.m}
